...
class Search_Model extends CI_Model {
    ...
    {
        $query=$this->db->query("SELECT town, date, connection_id, stops_id FROM connections_stops WHERE town='$from' AND date>'$date';");
    
        $arr = array();
        foreach($query->result() as $row )
        {
            $sql2=$this->db->query("SELECT town, date, connection_id, stops_id FROM connections_stops WHERE town='$to' AND connection_id='$row->connection_id';");
