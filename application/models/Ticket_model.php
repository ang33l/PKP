...
class Ticket_model extends CI_Model {
    ...
    public function quantity($numSeats, $id_connection, $id_start, $id_end)
    {
        ...
        $query=$this->db->query("SELECT train.train_id, (SUM(compartment.quantity_seats) - (SELECT COUNT(*) FROM (SELECT t.ticket_id, t.start, t.end, (SELECT date FROM connections_stops WHERE stops_id = t.start) AS date_start, (SELECT date FROM connections_stops WHERE stops_id = t.end) AS date_END FROM tickets t INNER JOIN connections_stops cs ON t.connection_id = cs.connection_id WHERE t.active=1 AND t.connection_id = $id_connection AND ((SELECT date FROM connections_stops WHERE stops_id = t.start) > (SELECT date FROM connections_stops WHERE stops_id = $id_start) AND (SELECT date FROM connections_stops WHERE stops_id = t.end) > (SELECT date FROM connections_stops WHERE stops_id = $id_end)) GROUP BY t.ticket_id) i ) - (SELECT COUNT(*) FROM (SELECT t.ticket_id, t.start, t.end, (SELECT date FROM connections_stops WHERE stops_id = t.start) AS date_start, (SELECT date FROM connections_stops WHERE stops_id = t.end) AS date_END FROM tickets t INNER JOIN connections_stops cs ON t.connection_id = cs.connection_id WHERE t.active=1 AND t.connection_id = $id_connection AND ((SELECT date FROM connections_stops WHERE stops_id = t.start) = (SELECT date FROM connections_stops WHERE stops_id = $id_start) AND (SELECT date FROM connections_stops WHERE stops_id = t.end) = (SELECT date FROM connections_stops WHERE stops_id = $id_end)) GROUP BY t.ticket_id) i) + (SELECT COUNT(*) FROM (SELECT t.ticket_id, t.start, t.end, (SELECT date FROM connections_stops WHERE stops_id = t.start) AS date_start, (SELECT date FROM connections_stops WHERE stops_id = t.end) AS date_END FROM tickets t INNER JOIN connections_stops cs ON t.connection_id = cs.connection_id WHERE t.active=1 AND t.connection_id = $id_connection AND ((SELECT date FROM connections_stops WHERE stops_id = $id_start) < (SELECT date FROM connections_stops WHERE stops_id = t.start) AND (SELECT date FROM connections_stops WHERE stops_id = $id_end) < (SELECT date FROM connections_stops WHERE stops_id = t.end)) GROUP BY t.ticket_id) i) ) AS wolne FROM train INNER JOIN train_carriage ON train.train_id=train_carriage.train_id INNER JOIN carriage ON train_carriage.carriage_id = carriage.carriage_id INNER JOIN carriage_compartment ON carriage.carriage_id = carriage_compartment.carriage_id INNER JOIN compartment ON compartment.compartment_id = carriage_compartment.compartment_id WHERE train.train_id = 1 GROUP BY train.train_id;");
        return $query->row();
    }
